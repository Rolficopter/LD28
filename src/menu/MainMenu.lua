...
function MainMenu:update(dt)
  ...
  if love.keyboard.wasPressed("s") or love.keyboard.wasPressed("down") then
    self.currentSelection = self.currentSelection + 1
    if self.currentSelection > 1 then self.currentSelection = 1 end
  end
...
function MainMenu:render()
  ...
  love.graphics.setColor({255, 255, 255, 255})
  love.graphics.rectangle("fill", love.graphics.getWidth() / 2 - math.max(self.fontHeadingArimo:getWidth(self.textStartGame), self.fontTextArimo:getWidth(self.textQuit)) / 2, love.graphics.getHeight() / 2 - 70,  math.max(self.fontHeadingArimo:getWidth(self.textStartGame), self.fontTextArimo:getWidth(self.textQuit)), self.fontTextArimo:getHeight() * 2 + 50)
